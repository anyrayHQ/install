...
The gateway always restricts `/v1` to enrolled developers — passwordless, offline
hardening: only devices whose `anyray-connect` run enrolled a per-device key against
the gateway's pinned trust anchor get through; everyone else gets a 403.
