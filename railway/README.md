...
3. Railway will prompt you to create a new project and enter the deployment
   token. All other secrets are auto-generated by Railway's template functions.
...
The template enforces usage metering — every Anyray deployment connects to the
Portal for content-free metering (see the main README). It sets
`ANYRAY_METERING_ENABLED=true`, auto-generates `ANYRAY_PSEUDONYM_SALT`, and asks
for the one value it cannot generate: `ANYRAY_DEPLOYMENT_TOKEN`.

| Service   | Variable                  | Value         | Note                        |
|-----------|---------------------------|---------------|-----------------------------|
| `gateway` | `ANYRAY_DEPLOYMENT_TOKEN` | `<adt_token>` | Required during deploy      |

> **Set the token during deployment.** Railway presents the empty token field as
> the one required operator prompt. `ANYRAY_METERING_ENABLED=true` is part of the
> supported template, not a user-facing toggle; `ANYRAY_PSEUDONYM_SALT` is
> auto-generated, so you only paste the token.

Once the gateway starts and phones home (~10s), the deployment flips to
