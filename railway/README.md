...
Capability-aware template revisions also ask the operator for one coordinated
rollout timestamp.
...
2. If Railway prompts for
   `ANYRAY_PERSISTENT_TRANSCRIPT_POLICY_ACTIVATE_AT`, choose one ISO-8601 UTC
   instant at least an hour in the future, formatted as
   `YYYY-MM-DDTHH:MM:SS.000Z`. Older published template revisions without the
   capability marker correctly omit this prompt.
3. Railway will prompt you to create a new project. Accept.
4. All secrets are auto-generated by Railway's template functions.
5. Wait for all services to deploy (~3 min). When step 2 applied, confirm both
   gateway and optimizer are healthy before the activation instant.
6. Open the `proxy` service → "Settings" → "Public Networking" → Generate domain
   (target port 80 — nginx listens there, not on $PORT).
   This is your console URL.
7. Open the `gateway` service → "Settings" → "Public Networking" → Generate domain.
...
one metering value. Capability-aware templates separately collect the rollout
timestamp during deploy. Get your
`<adt_token>` (`adt_…`) at <https://app.anyray.ai> →
...
| `ANYRAY_PERSISTENT_TRANSCRIPT_POLICY_ACTIVATE_AT` | Operator prompt in capability-aware templates | Shared future ISO instant; omitted by older template revisions and preserved after activation. |
...
Railway rolls gateway and optimizer independently. Before first reconciling a
capability-aware template revision, set
`ANYRAY_PERSISTENT_TRANSCRIPT_POLICY_ACTIVATE_AT` on the gateway to one shared
future instant with enough time for both services to deploy and roll back. Then
update both service images and verify both are healthy before that instant. If
either rollout fails, restore the old images before activation. Keep the exact
same timestamp on every later update, even after it is in the past.

For fresh one-click installs, `build-publish.sh` adds this required prompt when
the repository's persistent capability marker exists and the gateway,
optimizer, proxy, and IaC pins all match. An older install revision without the
marker omits the empty value. The pinned image version does not toggle the
prompt.

Do not substitute an already-past default during the first update. Mixed old and
new services can otherwise alternate cache-affecting request bytes.
...
- `<service>.vars` — the exact `KEY=VALUE` block to paste into each service's
  Raw Editor. The activation prompt is included when the install capability
  marker exists.
