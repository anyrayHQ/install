...
write_values_stub() {
  {
    ...
    echo "host: \"${HOST}\""
    echo ""
    echo "# Keep this deployment current on its own: images follow the moving"
    echo "# policy-stable channel (promoted every release) and the chart's"
    echo "# autoUpdate CronJob rolls them nightly. Set here rather than relying on"
    echo "# a chart default, so upgrading the chart never changes this behaviour"
    echo "# behind your back."
    echo "#"
    echo "# Comment the two lines out to pin a fixed, auditable build instead"
    echo "# (usual under GitOps): images then track the chart's appVersion and the"
    echo "# nightly roll is skipped automatically."
    echo "image:"
