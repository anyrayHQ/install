...
if [ "$K8S" -eq 0 ]; then
  ...
  # Reconcile: add only missing vars (existing values untouched). Derive the paired LF keys from whatever already exists so the obs/init/basic-auth trio stays in sync.
  LF_PUBLIC_KEY="$(get_env ANYRAY_OBSERVABILITY_PUBLIC_KEY)"
  [ -n "$LF_PUBLIC_KEY" ] || LF_PUBLIC_KEY="$(get_env ANYRAY_INIT_PROJECT_PUBLIC_KEY)"
  [ -n "$LF_PUBLIC_KEY" ] || LF_PUBLIC_KEY="pk-lf-$(hex 16)"
  LF_SECRET_KEY="$(get_env ANYRAY_OBSERVABILITY_SECRET_KEY)"
  [ -n "$LF_SECRET_KEY" ] || LF_SECRET_KEY="$(get_env ANYRAY_INIT_PROJECT_SECRET_KEY)"
  [ -n "$LF_SECRET_KEY" ] || LF_SECRET_KEY="sk-lf-$(hex 16)"

  add_if_missing ANYRAY_OBSERVABILITY_PUBLIC_KEY "$LF_PUBLIC_KEY"
  add_if_missing ANYRAY_OBSERVABILITY_SECRET_KEY "$LF_SECRET_KEY"
  add_if_missing ANYRAY_INIT_PROJECT_PUBLIC_KEY  "$LF_PUBLIC_KEY"
  add_if_missing ANYRAY_INIT_PROJECT_SECRET_KEY  "$LF_SECRET_KEY"
  add_if_missing ANYRAY_OBS_BASIC_AUTH           "$(b64enc "${LF_PUBLIC_KEY}:${LF_SECRET_KEY}")"
  ...
  add_if_missing NEXTAUTH_SECRET                 "$(b64)"
  add_if_missing SALT                            "$(b64)"
  add_if_missing ENCRYPTION_KEY                  "$(hex 32)"
  add_if_missing NEXTAUTH_URL                    "http://${HOST}:3000"
  add_if_missing TELEMETRY_ENABLED               "false"
  add_if_missing ANYRAY_INIT_ORG_ID              "anyray"
  add_if_missing ANYRAY_INIT_ORG_NAME            "Anyray"
  add_if_missing ANYRAY_INIT_PROJECT_ID          "anyray-gateway"
  add_if_missing ANYRAY_INIT_PROJECT_NAME        "Anyray Gateway"
  add_if_missing ANYRAY_INIT_USER_EMAIL          "admin@anyray.local"
  add_if_missing ANYRAY_INIT_USER_NAME           "Admin"
  add_if_missing ANYRAY_INIT_USER_PASSWORD       "$(hex 16)"
  add_secret     POSTGRES_PASSWORD               "$(hex 16)" anyray_anyray_postgres_data
  add_if_missing CLICKHOUSE_PASSWORD             "$(hex 16)"
  add_if_missing REDIS_AUTH                      "$(hex 16)"
  add_if_missing MINIO_ROOT_PASSWORD             "$(hex 16)"
  chmod 600 .env
